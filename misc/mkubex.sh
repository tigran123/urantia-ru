...
echo -E "<li title='<i>Представлено при содействии отряда личностей сверхвселенной, находящихся на Уверсе, действующего под руководством Древних Дней Орвонтона.</i>'><a href=\".U0_0_1\"><b>I: ЦЕНТРАЛЬНАЯ ВСЕЛЕННАЯ И СВЕРХВСЕЛЕННЫЕ</b></a>" >> $TOC
echo -E " <ul>" >> $TOC
...
do
   if ((i == 32)) ; then
      echo -E " </ul>" >> $TOC
      echo -E "</li>" >> $TOC
      echo -E "<li title='<i>Под покровительством Небадонского Отряда Личностей Локальной Вселенной, действующих на основе полномочий от Гавриила из Спасограда.</i>'><a href=\".U32_0_1\"><b>II: ЛОКАЛЬНАЯ ВСЕЛЕННАЯ</b></a>" >> $TOC
      echo -E " <ul>" >> $TOC
   fi
   if ((i == 57)) ; then
      echo -E " </ul>" >> $TOC
